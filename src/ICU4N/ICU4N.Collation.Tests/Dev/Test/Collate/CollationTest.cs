...
namespace ICU4N.Dev.Test.Collate
{
    public class CollationTest : TestFmwk
    {
        ...
        internal static String Prettify(SortKey key)
        {
            byte[] bytes = key.KeyData;
            return Prettify(bytes, bytes.Length);
        }

        internal static String Prettify(RawSortKey key)
        {
                                          ...
                                          String source, String target, int result)
        {
            ...
            SortKey ssk = myCollation.GetSortKey(source);
            SortKey tsk = myCollation.GetSortKey(target);
            ...
            RawSortKey srsk = new RawSortKey();
            myCollation.GetRawSortKey(source, srsk);
            RawSortKey trsk = new RawSortKey();
            myCollation.GetRawSortKey(target, trsk);
            compareResult = ssk.CompareTo(tsk);
            if (compareResult != result)
            {
        ...
        public void TestSubSequence()
        {
            ...
            UTF16CollationIterator ci1 = new UTF16CollationIterator(data, false, s.ToCharSequence(), 0);
            UTF16CollationIterator ci2 = new UTF16CollationIterator(data, false, s.ToCharSequence(), 2);
        ...
        public void TestFCD()
        {
            ...
            FCDUTF16CollationIterator u16ci = new FCDUTF16CollationIterator(data, false, s.ToCharSequence(), 0);
        ...
        private String PrintSortKey(SortKey key)
        {
            byte[] p = key.KeyData;
        ...
        private bool GetSortKey(String norm, String line, String s, out SortKey keyOut)
        {
            SortKey key = coll.GetSortKey(s);
            keyOut = key;

            byte[] keyBytes = key.KeyData;
            if (keyBytes.Length == 0 || keyBytes[keyBytes.Length - 1] != 0)
            {
                Logln(fileTestName);
                Logln(line);
                Logln(PrintSortKey(key));
         ...
         */
        private bool GetMergedSortKey(String s, out SortKey key)
        {
            key = null;
            SortKey mergedKey = null;
            ...
            for (int i = 0; ;)
            {
                ...
                SortKey tmpKey = coll.GetSortKey(s.Substring(segmentStart, i - segmentStart)); // ICU4N: Corrected 2nd parameter
                if (mergedKey == null)
                {
                    mergedKey = tmpKey;
                }
        ...
        private static int GetDifferenceLevel(SortKey prevKey, SortKey key,
                int order, bool collHasCaseLevel)
        {
            ...
            byte[] prevBytes = prevKey.KeyData;
            byte[] bytes = key.KeyData;
                                        ...
                                        int expectedOrder, int expectedLevel)
        {
            ...
            SortKey prevKeyOut = null;
            SortKey prevKey;
            if (!GetSortKey(norm, fileLine, prevString, out prevKeyOut))
            {
                return false;
            }
            prevKey = prevKeyOut;
            ...
            SortKey keyOut;
            SortKey key;
            if (!GetSortKey(norm, fileLine, s, out keyOut))
            {
                return false;
            }
            key = keyOut;
            ...
            SortKey outPrevKey;
            SortKey outKey;
            if (GetMergedSortKey(prevString, out outPrevKey) | GetMergedSortKey(s, out outKey))
            {
                prevKey = outPrevKey;
                key = outKey;
                order = prevKey.CompareTo(key);
