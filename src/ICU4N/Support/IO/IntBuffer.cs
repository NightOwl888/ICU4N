...
namespace ICU4N.Support.IO
{
    ...
    public abstract class Int32Buffer : Buffer, IComparable<Int32Buffer>
    {
        ...
        public static Int32Buffer Wrap(int[] array, int start, int length)
        {
            if (array == null)
            {
                throw new ArgumentNullException(nameof(array));
            }
            if (start < 0 || length < 0 || (long)length + (long)start > array.Length)
            {
        ...
        /// </summary>
        /// <param name="otherBuffer">Another <see cref="Int32Buffer"/>.</param>
        /// <returns>a negative value if this is less than <paramref name="otherBuffer"/>; 0 if this
        /// equals to <paramref name="otherBuffer"/>; a positive value if this is greater
        /// than <paramref name="otherBuffer"/>.</returns>
        ...
        public virtual Int32Buffer Get(int[] destination, int offset, int length)
        {
            int len = destination.Length;
            if (offset < 0 || length < 0 || (long)length + (long)offset > len)
            {
                throw new IndexOutOfRangeException();
            }
            if (length > Remaining)
        ...
        /// <summary>
        /// Returns an <see cref="int"/> at the specified index; the position is not changed.
        /// </summary>
        /// <param name="index">The index, must not be negative and less than limit.</param>
        /// <returns>An <see cref="int"/> at the specified index.</returns>
