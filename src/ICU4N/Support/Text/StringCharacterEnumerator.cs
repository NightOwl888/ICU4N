...
namespace ICU4N.Support.Text
{
    ...
    //            throw new ArgumentOutOfRangeException(string.Empty, $"{nameof(startIndex)}: {startIndex} + {nameof(length)}: {length} > {nameof(value.Length)}: {value.Length}");
    ...
    public sealed class StringCharacterEnumerator : ICharacterEnumerator
    {
        ...
        /// Gets the <see cref="char"/> at the current position.
        /// </summary>
        public char Current => position < 0 || position >= str.Length ? unchecked((char)(-1)) : str[position];
