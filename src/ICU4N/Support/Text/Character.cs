...
namespace ICU4N.Support.Text
{
    public class Character
    {
        ...
        /// is larger than <paramref name="endIndex"/>.
        /// </exception>
        internal static int CodePointCount(ICharSequence seq, int beginIndex, int endIndex)
        {
            int length = seq.Length;
            if (beginIndex < 0 || endIndex > length || beginIndex > endIndex)
            {
        ...
        {
            return CodePointBefore(new StringCharSequence(seq), index);
        }

        public static int CodePointBefore(StringBuilder seq, int index)
        {
            return CodePointBefore(new StringBuilderCharSequence(seq), index);
        }

        public static int CodePointBefore(char[] seq, int index)
        {
            return CodePointBefore(new CharArrayCharSequence(seq), index);
        }
