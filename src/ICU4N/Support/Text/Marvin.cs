    ...
    internal static partial class Marvin
    {
        ...
        InputTooSmallToEnterMainLoop:
            ...
            if ((count & 0b_0010) != 0)
            {
                ...
                {
                    partialResult |= (uint)Unsafe.ReadUnaligned<ushort>(ref data);
                    partialResult = (uint)BitOperation.RotateLeft((int)partialResult, 16);
                }
            }
        ...
        internal static void Block(ref uint rp0, ref uint rp1)
        {
            ...
            p1 ^= p0;
            p0 = (uint)BitOperation.RotateLeft((int)p0, 20);

            p0 += p1;
            p1 = (uint)BitOperation.RotateLeft((int)p1, 9);

            p1 ^= p0;
            p0 = (uint)BitOperation.RotateLeft((int)p0, 27);

            p0 += p1;
            p1 = (uint)BitOperation.RotateLeft((int)p1, 19);
