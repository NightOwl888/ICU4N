...
namespace ICU4N.Impl
{
    ...
    public class AnonymousCache<TKey, TValue, TData> : Cache<TKey, TValue, TData> where TValue : class
    {
        private readonly Func<TKey, TData, TValue> createInstance;
        public AnonymousCache(Func<TKey, TData, TValue> createInstance)
        {
            this.createInstance = createInstance ?? throw new ArgumentNullException(nameof(createInstance));
        }

        protected override TValue CreateInstance(TKey key, TData data)
        {
