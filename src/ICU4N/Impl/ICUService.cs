...
namespace ICU4N.Impl
{
    ...
    /// <para/>
    /// <see cref="ICUService"/> objects are instantiated by <see cref="IFactory"/> objects registered with
    /// the service.  The service queries each <see cref="IFactory"/> in turn, from most recently
    ...
    /// service.  When registered, a <see cref="IFactory"/> is installed at the head of
    ...
    /// <para/>
    /// <see cref="ICUService"/> uses <see cref="Key"/>s to query factories and perform
    /// fallback.  The <see cref="Key"/> defines the canonical form of the id, and
    /// implements the fallback strategy.  Custom <see cref="Key"/>s can be defined that
    /// parse complex IDs into components that <see cref="IFactory"/>s can more easily
    /// use.  The <see cref="Key"/> can cache the results of this parsing to save
    /// repeated effort.  <see cref="ICUService"/> provides convenience APIs that
    /// take Strings and generate default <see cref="Key"/>s for use in querying.
    /// <para/>
    ...
    /// <see cref="ICUService"/> by overriding it, for example, to customize the <see cref="Key"/> and
    ...
    public class ICUService : ICUNotifier
    {
        ...
        /// <para/>
        /// The default implementation of <see cref="Key"/> has no fallbacks and
        /// has no custom descriptors.
        /// </remarks>
        public class Key // ICU4N TODO: API - de-nest ?
        {
        ...
        /// a supported id.
        /// </summary>
        public interface IFactory // ICU4N TODO: API - de-nest ?
        {
            /// <summary>
            ...
            /// the service's <see cref="GetKey(Key, string[], IFactory)"/> method
        ...
        /// <see cref="UpdateVisibleIDs(IDictionary{string, IFactory})"/> to add a mapping from its ID to itself
        /// if visible is true, or to remove any existing mapping
        /// for its ID if visible is false.
        /// </summary>
        public class SimpleFactory : IFactory // ICU4N TODO: API - de-nest ?
        {
            ...
            /// <summary>
            /// Convenience constructor that calls <see cref="SimpleFactory.SimpleFactory(object, string, bool)"/>
            /// with visible true.
            /// </summary>
            ...
            /// the <paramref name="key"/>'s <see cref="Key.CurrentID"/>.  
        ...
        /// <summary>
        /// Convenience override for <see cref="GetKey(Key, string[])"/>.  This uses
        ...
        /// <summary>
        /// Convenience override for <see cref="GetKey(Key, string[])"/>.
        /// </summary>
        ...
        /// <summary>
        /// Register a <see cref="IFactory"/>.  Returns the factory if the service accepts
        ...
        /// <summary>
        /// Unregister a <see cref="IFactory"/>.  The first matching registered factory will
        ...
        /// <summary>
        /// Create a key from an id.  This creates a <see cref="Key"/> instance.
        ...
        /// <see cref="AcceptsListener(IEventListener)"/> and <see cref="NotifyListener(IEventListener)"/> as appropriate.
        /// </summary>
        public interface IServiceListener : IEventListener // ICU4N TODO: API - de-nest ?
        {
