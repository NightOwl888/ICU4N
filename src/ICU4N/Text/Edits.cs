...
namespace ICU4N.Text
{
    ...
    public sealed class Edits
    {
        ...
        internal const int MAX_UNCHANGED = MAX_UNCHANGED_LENGTH - 1;

        // 0mmmnnnccccccccc with m=1..6 records ccc+1 replacements of m:n text units.
        internal const int MAX_SHORT_CHANGE_OLD_LENGTH = 6;
        internal const int MAX_SHORT_CHANGE_NEW_LENGTH = 7;
        internal const int SHORT_CHANGE_NUM_MASK = 0x1ff;
        internal const int MAX_SHORT_CHANGE = 0x6fff;
        ...
        internal const int LENGTH_IN_1TRAIL = 61;
        internal const int LENGTH_IN_2TRAIL = 62;

        internal const int STACK_CAPACITY = 100;
        private char[] array;
        private int length;
        private int delta;
        ...
        /// <summary>
        /// Returns an <see cref="EditsEnumerator"/> for coarse-grained changes for simple string updates.
        /// Skips non-changes.
        /// </summary>
        /// <returns>An <see cref="EditsEnumerator"/> that merges adjacent changes.</returns>
        /// <draft>ICU 59</draft>
        /// <provisional>This API might change or be removed in a future release.</provisional>
        public EditsEnumerator GetCoarseChangesEnumerator()
        {
            return new EditsEnumerator(array, length, true, true);
        }
        /// <summary>
        /// Returns an <see cref="EditsEnumerator"/> for coarse-grained changes and non-changes for simple string updates.
        /// </summary>
        /// <returns>An <see cref="EditsEnumerator"/> that merges adjacent changes.</returns>
        /// <draft>ICU 59</draft>
        /// <provisional>This API might change or be removed in a future release.</provisional>
        public EditsEnumerator GetCoarseEnumerator()
        {
            return new EditsEnumerator(array, length, false, true);
        }

        /// <summary>
        /// Returns an <see cref="EditsEnumerator"/> for fine-grained changes for modifying styled text.
        /// Skips non-changes.
        /// </summary>
        /// <returns>An <see cref="EditsEnumerator"/> that separates adjacent changes.</returns>
        /// <draft>ICU 59</draft>
        /// <provisional>This API might change or be removed in a future release.</provisional>
        public EditsEnumerator GetFineChangesEnumerator()
        {
            return new EditsEnumerator(array, length, true, false);
        }

        /// <summary>
        /// Returns an <see cref="EditsEnumerator"/> for fine-grained changes and non-changes for modifying styled text.
        /// </summary>
        /// <returns>An <see cref="EditsEnumerator"/> that separates adjacent changes.</returns>
        /// <draft>ICU 59</draft>
        /// <provisional>This API might change or be removed in a future release.</provisional>
        public EditsEnumerator GetFineEnumerator()
        {
            return new EditsEnumerator(array, length, false, false);
        }
        ...
        public Edits MergeAndAppend(Edits ab, Edits bc)
        {
            ...
            EditsEnumerator abIter = ab.GetFineEnumerator();
            EditsEnumerator bcIter = bc.GetFineEnumerator();
    ...
    /// </summary>
    /// <seealso cref="Edits.GetCoarseEnumerator()"/>
    /// <seealso cref="Edits.GetFineEnumerator()"/>
    /// <draft>ICU 59</draft>
    /// <provisional>This API might change or be removed in a future release.</provisional>
    public sealed class EditsEnumerator // ICU4N specific - de-nested and renamed from Edits.Enumerator
    {
        ...
        internal EditsEnumerator(char[] a, int len, bool oc, bool crs)
        {
            array = a;
            length = len;
            onlyChanges_ = oc;
        ...
        private bool MoveNext(bool onlyChanges)
        {
            ...
            if (u <= Edits.MAX_UNCHANGED)
            {
                ...
                changed = false;
                oldLength_ = u + 1;
                while (index < length && (u = array[index]) <= Edits.MAX_UNCHANGED)
                {
                    ++index;
                    oldLength_ += u + 1;
                }
            ...
            {
                int oldLen = u >> 12;
                int newLen = (u >> 9) & Edits.MAX_SHORT_CHANGE_NEW_LENGTH;
                int num = (u & Edits.SHORT_CHANGE_NUM_MASK) + 1;
                if (coarse)
                {
            ...
            while (index < length && (u = array[index]) > Edits.MAX_UNCHANGED)
            {
                ...
                if (u <= Edits.MAX_SHORT_CHANGE)
                {
                    int num = (u & Edits.SHORT_CHANGE_NUM_MASK) + 1;
                    oldLength_ += (u >> 12) * num;
                    newLength_ += ((u >> 9) & Edits.MAX_SHORT_CHANGE_NEW_LENGTH) * num;
                }
                else
                {
                    Debug.Assert(u <= 0x7fff);
        ...
        private bool Previous()
        {
            ...
            if (remaining > 0)
            {
                // Fine-grained iterator: Continue a sequence of compressed changes.
                int u2 = array[index];
                Debug.Assert(Edits.MAX_UNCHANGED < u2 && u2 <= Edits.MAX_SHORT_CHANGE);
                if (remaining <= (u2 & Edits.SHORT_CHANGE_NUM_MASK))
                {
                    ++remaining;
            ...
            if (u <= Edits.MAX_UNCHANGED)
            {
                ...
                changed = false;
                oldLength_ = u + 1;
                while (index > 0 && (u = array[index - 1]) <= Edits.MAX_UNCHANGED)
                {
                    --index;
                    oldLength_ += u + 1;
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    Debug.Assert(u > Edits.MAX_SHORT_CHANGE);
                    int headIndex = index++;
                    oldLength_ = ReadLength((u >> 6) & 0x3f);
                    newLength_ = ReadLength(u & 0x3f);
                    index = headIndex;
                ...
                if (i >= (spanStart / 2))
                {
                    ...
                    for (; ; )
                    {
                        ...
                        if (remaining > 0)
                        {
                            ...
                            int u = array[index];
                            Debug.Assert(Edits.MAX_UNCHANGED < u && u <= Edits.MAX_SHORT_CHANGE);
                            int num = (u & Edits.SHORT_CHANGE_NUM_MASK) + 1 - remaining;
