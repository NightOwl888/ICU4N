...
namespace ICU4N.Dev.Test.Translit
{
    public sealed class TestUtility
    {
        ...
        {
            String foo = Convert.ToString(ch, 16).ToUpperInvariant();
            return "0000".Substring(0, 4 - foo.Length) + foo; // ICU4N: Checked 2nd substring parameter
        }

        public static String Hex(int ch)
        {
            String foo = Convert.ToString(ch, 16).ToUpperInvariant();
            return "00000000".Substring(0, 4 - foo.Length) + foo; // ICU4N: Checked 2nd substring parameter
        }
